...
var selctedAmt = 0; //Obtain sel from candidates
...
const getTodayDateStr = () => {
  ...
  todayString = `${currDate.getFullYear()}-${todayMonth}-${todayDate}`

  console.log("Date Called", todayString);
}
...
const Mix = () => {
  ...
  const startDateOnChange = (newValue) => {
    ...
    recvData_startDate = `${Object.values(newValue)[4]}-${month}-${
      Object.values(newValue)[6]
    }`;
  ...
  const endDateOnChange = (newValue) => {
    ...
    recvData_endDate = `${Object.values(newValue)[4]}-${month}-${
      Object.values(newValue)[6]
    }`;
  ...
  const handleDialogClose = (event) => {
    ...
    // console.log("Rerendering ready here");
    setOpen(false);
      ...
      for (let mode of excList_SQL_list) {
        ...
        for (let exe of mode) {
          ...
          if (Object.keys(exe.Exercise).length !== 0) {
            ...
            tmplt.weight = exe.Schedule[0].SubSchedule.Weight;
  ...
  //************************* Add onClick Function (Candidates to Schedule) ********************************************************* */

  var init_addCall = true; //Since the Promise will recursively call add() to await input
  var counter = 0;
  var modeIndex; //Index for ROWS_list

  async function addCand2Sche_toSQL() {

    //Once got ONE event user input, 
    //reset the Promise Control to avoid the case in which the next schedule takes previous user input
    gotAllInput = false 
  ...
  async function addCand2Sche_Dialog(element) {
    ...
    //Local Variable
    var dup = false; //Detect if the target exercise is exsited in the schedule
        
    var selcted = []; //Obtain sel from candidates


    //Detect if there are multiply selection
    selctedAmt = rowSelIndex.length - 1



    //Loop termination control for development
    counter++;
    // if(counter > 10){
    //   return
    // }
    ...
    // console.log(rowSelIndex.length, "!!!!!!");

    //Filling up selection list
    ...
    p.then(() => {
      ...
      var tmpSche_start = recvData_startDate;
      var tmpSche_end = recvData_endDate;
      var tmpSche_repe = recvData_repeDialog;
      var tmpSche_sets = recvData_setsDialog;
      var tmpSche_weight = recvData_weightDialog;
      var tmpSche_duration = recvData_durationDialog;


      //Reset variables for next event input
      recvData_startDate = "";
      recvData_endDate = "";
      recvData_repeDialog = "";
      recvData_setsDialog = "";
      ...
      if(recvData_startDate === ""){
        tmpSche_start = todayString
      }

      if(recvData_endDate === ""){
        tmpSche_end = todayString
      }
      ...
      for (let evt of selcted) {
        ...
        for (let i = 0; i < ROWS_list[modeIndex].length; i++) {
          if (
            ROWS_list[modeIndex][i].exercise_name ===
              FE_row_OBJ.exercise_name &&
        ...
        console.log(
          "Ready to add to SQL",
          Object.values(drillDetail_OBJ_SQL)[modeIndex].Events
        );
      }
  ...
  //************************* Add onClick Function Definition Ends Here (Candidates to Schedule) ********************************************************* */
  ...
  async function handleDeletefromShce(element) {
    ...
    // if(thisModeName==="ModeName1"){
    //   console.log("YES");
    //   thisMode_OBJ.Events = []
      
    // }

    // console.log(thisMode_OBJ);
    ...
    for (let i = 0; i < rowSelID_Ctnt.length - 1; i++) {
      ...
      ROWS_list[modeIndex] = ROWS_list[modeIndex].filter(
        (ele) => ele.id !== rowSelID_Ctnt[i]
      );
    }
    ...
    for (let i = 0; i < targetEle_List.length; i++) {
      ...
      thisMode_OBJ.Events = thisMode_OBJ.Events.filter(
        (ele) =>
          ele.Exercise.ExerciseTitle !== targetEle_List[i][0].exercise_name
      );
    }
