...
import { user } from "../../data/sql/dbConfig";
...
const Mix = () => {
  ...
  async function addCand2Sche_Dialog(element) {
    ...
    var input_delay = 5000 //ms
    

  
    for (let index = 0; index < selcted.length; index++) {
      ...
      //   // setDialog_ReRender([...dialog_ReRender, cnt])
      //   setOpen(false)
    ...
    modeName_Dialog.current = thisModeName
    ...
    for (let evt of selcted) {
      ...
      for (let i = 0; i < ROWS_list[modeIndex].length; i++) {
        if (
          ROWS_list[modeIndex][i].exercise_name === FE_row_OBJ.exercise_name &&
          ...
          dup = true;
          alert(` ${ROWS_list[modeIndex][i].exercise_name} in ${thisModeName} Duplicated`);
        }
